...
**Синхронизация видео со звуком** — мастер-часы плеера идут по звуку:
`AudioPlayer::playback_clock_sec` — это якорь последнего колбэка вывода
(«в такой-то момент будет слышно столько-то сэмплов», с поправкой на задержку
тракта: timestamp колбэка cpal, `calculate_latency_millis` у oboe) плюс время,
прошедшее с него. Считать часы просто по счётчику отданных сэмплов нельзя:
он стоит и прыгает ступенькой размером с буфер устройства (42 мс при 2048
сэмплах), и видео показывает кадры пачками — ровно те «рывки», которых нет
ни в UI, ни в декодере. Кадр показывается, когда его время пришло, с допуском
в половину периода опроса (`VideoPlayer::poll_frame` меряет его сам, это кадр
UI), а не заранее.
...
относительно часов и шагом самих часов. Ровная каденция выглядит так:
интервал avg ≈ 1/fps при разбросе в период кадра UI, опережение около нуля,
«часы стояли» 0 %.
