...
### 3.8. Масштаб интерфейса

```rust
use syngui::prelude::{set_ui_scale, ui_scale, MIN_UI_SCALE, MAX_UI_SCALE};

App::new().ui_scale(1.25)                // стартовое значение (обычно из настроек)
set_ui_scale(ui_scale() + 0.1);          // в рантайме, применяется следующим кадром
```

Множитель поверх системного DPI: при 1.25 всё приложение крупнее, а
логический вьюпорт — в 1.25 раза меньше, поэтому `viewport_below` и вся
адаптивная раскладка перестраиваются так же, как при уменьшении окна.
Диапазон — `MIN_UI_SCALE..=MAX_UI_SCALE` (0.5–3.0), значение приложение
хранит и восстанавливает само (`syngui/src/scale.rs`).
...
TableView::new(columns, rows)
    ...
    .column_order_state(..).reorderable_columns(true).on_column_reorder(|order| {})
...
Окно не бывает больше вьюпорта и не теряется за его краем: размер зажимается
по главному окну, позиция — так, чтобы полоса заголовка оставалась в
пределах экрана. Если заголовок всё же недостижим (окно приложения резко
сузилось, вырос масштаб интерфейса), окно тащится за тело — как с
`drag_on_body`, но только пока заголовок не схватить.
...
Тень шелла (`box-shadow`) должна гаснуть внутри того же `inset`: тень доходит
до `blur` px от края (гаусс, σ = blur/3) плюс смещение, а за границей
поверхности окна ничего не рисуется — не уместившийся хвост обрежется резким
прямоугольником. Правило: `blur + |offset| ≤ inset` (при `inset` 32 — `0 8 24`,
а `0 16 48` потребует отступа 64).
...
`syngui::app::request_tick` — UI не рендерится на каждый кадр видео. Если после
перемотки декодер отстаёт от аудио-часов, самый свежий опоздавший кадр
показывается сразу (иначе экран стоял бы чёрным при идущем звуке).

**Конвейер кадра на десктопе** (`HwAccel::Nvdec`/`Vaapi` и т.п.): декодер
отдаёт кадр в памяти GPU, дальше `av_hwframe_transfer_data` выгружает его в
ОЗУ (4K — ~1,5 ms) и `Scaler` переводит в RGBA для текстуры. Второй шаг и
есть потолок частоты кадров: из GPU кадр приходит в **NV12**, а у swscale для
NV12 → RGBA нет быстрого пути — 4K в один поток стоит ~13 ms против ~3 ms для
YUV420P. Поэтому `Scaler` режет кадр на полосы по числу ядер (до 8, каждая со
своим контекстом swscale — он не потокобезопасен) и считает их параллельно:
полоса конвертируется как самостоятельное изображение со смещением плоскостей
(срезы одного контекста swscale принимает только подряд сверху вниз). Плюс
выходные буферы ходят по кругу: RGBA-кадр 4K — 33 МБ, и выделять их заново
дороже самой конверсии (page fault на каждую страницу). Итог на 4K: 23 ms/кадр
→ 2,7 ms. Профиль этапов — `SYNGUI_VIDEO_STATS=1`, строка `[VIDEO PIPE]`.
Дальше резерв один: отдавать в GPU сами плоскости NV12 и конвертировать цвет
в шейдере — тогда с CPU уходит и конверсия, и 33 МБ трафика на кадр.

**Синхронизация видео со звуком** — мастер-часы плеера идут по звуку и в
шкале pts видео: pts первого сэмпла, ушедшего в текущий аудио-канал
(`VideoDecoder::audio_base_pts_sec`), плюс `AudioPlayer::playback_clock_sec`.
Последнее — якорь последнего колбэка вывода («в такой-то момент будет слышно
столько-то сэмплов», с поправкой на задержку тракта: timestamp колбэка cpal,
`calculate_latency_millis` у oboe) плюс время, прошедшее с него. Ловушки, на
которых это уже ломалось:
- счётчик отданных сэмплов стоит и прыгает ступенькой размером с буфер
  устройства (42 мс при 2048 сэмплах) — видео показывает кадры пачками;
- отсчёт от нуля или от цели перемотки вместо pts звука: поток начинается не
  с нуля (HLS — 0,119 с у звука), а перемотка встаёт на ключевой кадр раньше
  цели (в HLS — начало сегмента, до 10 с). Картинка уезжала от звука на эту
  разницу. Теперь перемотка точная: декодер до цели декодирует, но не
  отдаёт ни кадры, ни звук (граничный аудио-кадр обрезается по сэмплам);
- сэмплы обязаны идти вровень с pts: дыру в звуке декодер заполняет тишиной,
  перекрытие обрезает (`AudioTimeline`). Иначе звук кончается раньше
  картинки, видео-очередь полна, часы стоят — и воспроизведение встаёт;
- `resampling::Context::run` из ffmpeg-next выделяет выходной кадр размером
  входа: при 44,1 → 48 кГц 8 % сэмплов застревали в swr. `Resampler` сам
  выделяет выход по `swr_get_out_samples`.
Кадр показывается, когда его время пришло, с допуском в половину периода
опроса (`VideoPlayer::poll_frame` меряет его сам, это кадр UI), а не заранее;
пока звук в новый канал не пошёл (до 10 с), кадры не показываются.
Диагностика: `SYNGUI_VIDEO_STATS=1` — раз в секунду в лог уходит
`[VIDEO STATS]` с интервалами между показанными кадрами, опережением кадра
относительно часов, шагом самих часов и «кадр−звук» — pts показанного кадра
минус pts слышимого звука (> 0 — картинка впереди). `[AUDIO SYNC]` — pts
первого звука после открытия/перемотки. Норма: интервал avg ≈ 1/fps при
разбросе в период кадра UI, опережение и кадр−звук около нуля, «часы стояли»
0 %.

**Сворачивание приложения** — `Event::AppSuspended` / `Event::AppResumed`
рассылаются всем элементам, `EventHook::on_suspend` / `on_resume` — хуки для
приложения. На Android в фоне отнимают поверхность окна, видео-Surface и
аудио-поток: плеер не выживает, поэтому в `on_suspend` запоминайте позицию и
закрывайте `VideoPlayer`, в `on_resume` открывайте заново и делайте `seek`
(эталон `tv_rezka::screens::player::{suspend, resume}`). Декодер, у которого
кодек умер (200 ошибок `send_packet` подряд), сам завершает поток, а drop
`VideoDecoder` не зависает на полной очереди кадров.

**Конец ролика** — `VideoPlayer::is_ended()`: демуксер дошёл до EOF (флаг
`VideoDecoder::reached_eof`, перемотка его сбрасывает), и кадров нет дольше
секунды — очередь пуста или звук кончился раньше картинки и часы встали; на
паузе — `false`. `is_buffering` после EOF спиннер не просит. У HLS EOF бывает
и преждевременным (сегмент не скачался после повторов), поэтому «досмотрено»
сверяйте ещё с `position_sec`/`duration_sec` (эталон — автозапуск следующей
серии в `tv_rezka::screens::player`: EOF не раньше чем за 60 с до конца).

**Миниатюры перемотки** — `Image::crop(x, y, w, h)` показывает область
спрайт-листа (пиксели исходной картинки), пропорции fit — от области.
Эталон: `tv_rezka::thumbs` разбирает WebVTT (`url.jpg#xywh=…`), превью над
ползунком показывается, пока цель перемотки копится (debounce 500 мс после
последнего нажатия пульта — сегмент у источника запрашивается один раз).

**Батчинг** (`render/batcher`): открытые батчи группы хранятся в порядке
отрисовки, примитив уходит в ближайший с конца батч с тем же ключом
(пайплайн + текстура + клип), если между ними нет батча, чей bbox его
пересекает — порядок художника сохраняется без flush на `PushClip`/`PopClip`/
`ZBarrier`. Внешние тени (`Shadow`) собираются в первый теневой батч группы и
не блокируют слияние. Клип — uniform-слот на батч (до 256 за кадр) + scissor;
вариант с клипом в вершинах на Mali проигрывал из-за двух лишних varying на
фрагмент. Rebuild больше не взводит `force_full_measure`: scroll-culling
(`scroll_estimate`) проверяет хэш ограничений и берёт кэш только для
неизменившихся детей.
...
в профиле это `present`):
- полноэкранный слой ≈ 2 млн фрагментов — фон под непрозрачной картинкой,
  несколько градиентов и tint поверх неё. Затемнение сворачивайте в стопы
  одного градиента, а слои с картинкой рисуйте только там, где они видны
  (эталон `tv_rezka`: `.backdrop-gap`/`.backdrop-box` ограничивают фон
  областью 1440×918, стопы пересчитаны так, чтобы у края области градиент
  был непрозрачным и не давал шва);
- `box-shadow` — квад, расширенный на радиус размытия, с `exp` на каждый
  фрагмент: тень 0 10px 30px у карточки 200×300 ≈ 110k фрагментов, у 27
  карточек ≈ 3 млн. На тёмном фоне тень нефокусных элементов неразличима —
  оставляйте её только элементу в фокусе;
- `text-shadow` с размытием ≥ 6px — шейдер берёт до 169 выборок атласа на
  фрагмент; для читаемости достаточно 1–2px или затемнённого фона;
- `filter: blur` — 57 выборок на фрагмент на проход, в полный экран — сотни мс.
Эффект правок в tv_rezka: present при навигации 27–34 → 17–21 ms.
