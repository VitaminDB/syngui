...
impl ElementTree {
    ...
    pub(super) fn measure_column(
        ...
        let (explicit_w, explicit_h, min_w, max_w_dim, min_h, max_h_dim) =
            if let Some(node) = self.elements.get(&id) {
                let (ew, eh) = node.element.explicit_dimensions(
                    constraints.containing_block.width,
                    constraints.containing_block.height,
                );
                let (minw, maxw, minh, maxh) = node.element.min_max_dimensions(
                    constraints.containing_block.width,
                    constraints.containing_block.height,
                );
        ...
        let effective_max_height = if raw_h.is_infinite() {
        ...
        let child_cb = Size::new(
            if effective_max_width.is_finite() {
                effective_max_width
            } else {
                constraints.containing_block.width
            },
            if effective_max_height.is_finite() {
        ...
        let child_probes: Vec<ChildProbe> = children_idx
            .iter()
            .map(|&cidx| {
                if let Some(n) = self.elements.get_by_idx(cidx) {
                    let hint = n.hint_cache.clone();
                    let margin = if n.mss_margin_set {
                        n.mss_margin
                    } else {
                        n.element.margin()
                    };
        ...
        if let Some(max) = max_w_dim {
            width = width.min(max);
        }
        if let Some(min) = min_h {
            height = height.max(min);
        }
        if let Some(max) = max_h_dim {
        ...
        layout_log!(
            self,
            "Column: {} children ({}exp), size={:.1}x{:.1}",
            children_idx.len(),
            expanded_idx.len(),
            width,
            height
    ...
    pub(super) fn measure_row(
        ...
        if !expanded_idx.is_empty() && effective_max_width.is_finite() {
            ...
            let stretch =
                cross_align == CrossAxisAlignment::Stretch && effective_max_height.is_finite();
            for (cidx, flex) in &expanded_idx {
    ...
    pub(super) fn measure_tab_bar(
        ...
        let child_cb = Size::new(
            if per_child.is_finite() {
                per_child
            } else {
                constraints.containing_block.width
            },
            if effective_max_h.is_finite() {
                effective_max_h
            } else {
                constraints.containing_block.height
            },
    ...
    pub(super) fn measure_stack(
        ...
        let expanded = Constraints {
            min_width: if constraints.max_width.is_finite() {
                constraints.max_width
            } else {
                constraints.min_width
            },
            max_width: constraints.max_width,
            min_height: if constraints.max_height.is_finite() {
                constraints.max_height
            } else {
    ...
    pub(super) fn measure_center(
        ...
        let child_max_w = if element_size.width > 0.0 {
            element_size.width
        } else {
            constraints.max_width
        };
        let child_max_h = if element_size.height > 0.0 {
            element_size.height
        } else {
            constraints.max_height
        };
        ...
        layout_log!(
            self,
            "Center: element={:.1}x{:.1} child={:.1}x{:.1}",
            element_size.width,
            element_size.height,
            max_width,
            max_height
    ...
    pub(super) fn measure_container(
        ...
        let (explicit_w, explicit_h, min_w, max_w, min_h, max_h) =
            if let Some(node) = self.elements.get(&id) {
                let (ew, eh) = node.element.explicit_dimensions(
                    constraints.containing_block.width,
                    constraints.containing_block.height,
        ...
        let parent_imposed_w =
            constraints.max_width.is_finite() && constraints.min_width >= constraints.max_width;
        let parent_imposed_h =
            constraints.max_height.is_finite() && constraints.min_height >= constraints.max_height;
        let (mss_intrinsic_w, mss_intrinsic_h) = if let Some(node) = self.elements.get(&id) {
            let mss = node.element.mss();
            let iw = mss
                .and_then(|m| m.width)
                .map(|d| d.is_intrinsic())
                .unwrap_or(false);
            let ih = mss
                .and_then(|m| m.height)
                .map(|d| d.is_intrinsic())
        ...
        let self_cb_w = if let Some(m) = max_w {
            m
        ...
        let self_cb_h = if let Some(m) = max_h {
            m
        } else if let Some(eh) = explicit_h {
            eh
        } else {
            constraints.containing_block.height
        };
        let probe_cb = Size::new((self_cb_w - pad_h).max(0.0), (self_cb_h - pad_v).max(0.0));
        ...
        let (mut width, mut height, mut content) = if needs_intrinsic_pass {
            ...
            let final_constraints = Constraints {
                min_width: 0.0,
                max_width: final_max_w,
                min_height: final_min_h,
                max_height: child_max_h,
        ...
        layout_log!(
            self,
            "Container: {:.1}x{:.1} children={:.1}x{:.1} shrink=({},{})",
            width,
            height,
            content.width,
            content.height,
    ...
    pub(super) fn measure_portal(
        ...
        layout_log!(
            self,
            "Portal: children={:.1}x{:.1}, takes zero space",
            content_size.width,
            content_size.height
        );
        Size::zero()
    ...
    pub(super) fn measure_floating_window(
        &mut self,
        children: &[ElementId],
        constraints: Constraints,
        id: ElementId,
        ...
        layout_log!(
            self,
            "FloatingWindow: children={:.1}x{:.1}, takes zero space",
            content_size.width,
            content_size.height
        );
        Size::zero()
    ...
    pub(super) fn measure_tooltip(
        ...
        let tight = Constraints {
            ...
            max_width: target_size.width,
            min_height: target_size.height,
            max_height: target_size.height,
    ...
    pub(super) fn measure_flex(
        ...
        let total_row_gap = if lines.len() > 1 {
            row_gap * (lines.len() - 1) as f32
        } else {
            0.0
        };
        ...
        layout_log!(
            self,
            "Flex: {} children, {} lines, size={:.1}x{:.1}",
            children.len(),
            lines.len(),
            result_width,
            total_height
    ...
    pub(super) fn measure_grid(
        ...
        if let Some(node) = self.elements.get_mut(&id) {
            if let Some(grid) = node
                .element
                .as_any_mut()
                .and_then(|a| a.downcast_mut::<crate::widgets::containers::grid::GridElement>())
            {
        ...
        let total_height = if masonry {
            ...
            let total_row_gap = if row_heights.len() > 1 {
                row_gap * (row_heights.len() - 1) as f32
            } else {
                0.0
            };
        ...
        let final_size = Size::new(grid_width, total_height.min(constraints.max_height));
        ...
        layout_log!(
            self,
            "Grid: {}cols, {}masonry, size={:.1}x{:.1}",
            cols,
            if masonry { "masonry" } else { "standard" },
            final_size.width,
            final_size.height
        );
    ...
    pub(super) fn measure_horizontal_pages(
        &mut self,
        children: &[ElementId],
        constraints: Constraints,
        id: ElementId,
        ...
        layout_log!(
            self,
            "HorizontalPages: {} pages, size={:.1}x{:.1}",
            children.len(),
            width,
            height
        );
    ...
    pub(super) fn measure_split(
        ...
        let total_h = if constraints.max_height.is_finite() {
            constraints.max_height
        } else {
            300.0
        };
        ...
        if children.len() >= 2 {
            if horizontal {
                ...
                let c1 = Constraints {
                    ...
                    min_height: total_h,
                    max_height: total_h,
                    containing_block: Size::new(first_w, total_h),
                };
                let c2 = Constraints {
                    min_width: second_w,
                    max_width: second_w,
                    min_height: total_h,
                    max_height: total_h,
                    containing_block: Size::new(second_w, total_h),
                };
                ...
                let c1 = Constraints {
                    ...
                    min_height: first_h,
                    max_height: first_h,
                    containing_block: Size::new(total_w, first_h),
                };
                let c2 = Constraints {
                    min_width: total_w,
                    max_width: total_w,
                    min_height: second_h,
                    max_height: second_h,
                    containing_block: Size::new(total_w, second_h),
                };
                self.measure_recursive(children[0], c1);
                self.measure_recursive(children[1], c2);
            }
    ...
    pub(super) fn measure_scroll(
        ...
        if let Some(node) = self.elements.get_mut(&id) {
            ...
            node.element
                .set_content_size(Size::new(content_width, content_height));
        }

        layout_log!(
            self,
            "Scroll: viewport={:.1}x{:.1} content={:.1}x{:.1}",
            width,
            height,
            content_width,
            content_height
    ...
    pub(super) fn measure_positioned(
        &mut self,
        children: &[ElementId],
        constraints: Constraints,
        id: ElementId,
    ...
    pub(super) fn measure_pan_zoom(
        ...
        let tight = Constraints {
            min_width: viewport_w,
            max_width: viewport_w,
            min_height: viewport_h,
            max_height: viewport_h,
            containing_block: viewport_size,
        };
        if let Some(node) = self.elements.get_mut(&id) {
        ...
        layout_log!(
            self,
            "PanZoom: viewport={:.1}x{:.1} content={:.1}x{:.1}",
            viewport_w,
            viewport_h,
            content.width,
            content.height
    ...
    pub(super) fn measure_animated_size(
        ...
        layout_log!(
            self,
            "AnimatedSize: target={:.1}x{:.1} animated={:.1}x{:.1}",
            child_size.width,
            child_size.height,
            result.width,
            result.height
