...
pub(crate) fn clamp_finite(size: Size, constraints: Constraints) -> Size {
    ...
    let clamped_w = width.clamp(
        constraints.min_width.min(constraints.max_width),
        constraints.max_width,
    );
    let clamped_h = height.clamp(
...
pub(crate) fn clamp_finite_explicit(
    size: Size,
    ...
    let clamped_w = if explicit.0.is_some() {
        width
    } else {
        width.clamp(
            constraints.min_width.min(constraints.max_width),
    ...
    let clamped_h = if explicit.1.is_some() {
        height
    } else {
        height.clamp(
            constraints.min_height.min(constraints.max_height),
