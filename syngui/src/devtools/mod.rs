...
impl DevTools {
    ...
    pub fn handle_mouse_event(
        ...
        match event {
            Event::MouseDown {
                ...
                if pos.x >= content.origin.x
                    && pos.x <= content.origin.x + content.size.width
                    ...
                    && pos.y <= content.origin.y + content.size.height
                {
                    match self.active_tab {
                        DevToolsTab::Inspector => {
                            if self.picking_mode
                                && pos.y
                                    < content.origin.y - self.tree_scroll_offset
                                        + panel::LINE_HEIGHT
                                        + 4.0
                            {
                                self.picking_mode = false;
                        ...
                        DevToolsTab::EventLog => {
                            if pos.y >= content.origin.y
                                && pos.y < content.origin.y + panel::LINE_HEIGHT
                            {
    ...
    pub fn build_display_list(
        ...
        if surface_size.width <= 0.0 || surface_size.height <= 0.0 {
            return;
        }
