...
impl ClipRect {
    ...
    /// Границы клипа лежат ровно на границах физических пикселей — ножницы
    /// обрезают точно, и шейдерная маска не нужна.
    pub fn is_pixel_aligned(&self, scale: f32) -> bool {
        let aligned = |v: f32| {
            let px = v * scale;
        ...
        aligned(self.x())
            && aligned(self.y())
            && aligned(self.x() + self.width())
            && aligned(self.y() + self.height())
    }
...
///
/// Пиксель попадает внутрь, если внутри прямоугольника лежит его центр.
/// Прямоугольный клип — это ножницы, то есть целые пиксели: краевой пиксель
/// либо целиком внутри, либо целиком снаружи. Заливки, которые такой клип
/// обрезает, растягиваются на пиксель наружу (`Batcher::set_clip_expand`) —
/// иначе на этом пикселе верхний слой закрывал бы нижний лишь частично и
/// по краю оставалась бы полоска.
pub fn scissor_px(
    rect: [f32; 4],
    scale: f32,
    phys_w: u32,
    ...
    // Полоска тоньше половины пикселя иначе исчезла бы совсем.
    if sr == sx && sx < phys_w {
        sr = sx + 1;
    }
    if sb == sy && sy < phys_h {
...
mod tests {
    ...
    fn scissor_rounds_to_pixel_centers() {
        // 480 лог. × 1.334 = 640.3: центр пикселя 640 внутри клипа, значит
        // пиксель его, а 639 — уже нет.
        let clip = ClipRect::new(480.0, 0.0, 1440.0, 918.0);
        let (x, _y, w, _h) = clip.scissor(1.3340281, 2560, 1484).unwrap();
        assert_eq!(x, 640);
        assert_eq!(x + w, 2560);
    }

    #[test]
    fn dpi_aligned_clip_needs_no_expansion() {
        let clip = ClipRect::new(480.0, 0.0, 1440.0, 918.0);
        assert!(
            clip.is_pixel_aligned(1.0),
            "1:1 — границы ровно на пикселях"
        );
        assert!(clip.is_pixel_aligned(2.0), "целое кратное — тоже");
        assert!(
            !clip.is_pixel_aligned(1.0666667),
            "дробный масштаб уводит границу между пикселями"
        );
    }
