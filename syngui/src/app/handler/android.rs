...
impl AppHandler {
    ...
    unsafe fn query_safe_area_jni(
        ...
        let resources = env
            ...
            .l()
            .map_err(|e| format!("getResources cast: {}", e))?;

        let dimen_str: JObject = env
            .new_string("dimen")
            .map_err(|e| format!("new_string: {}", e))?
            .into();
        let android_str: JObject = env
            .new_string("android")
            .map_err(|e| format!("new_string: {}", e))?
            .into();

        let status_name: JObject = env
        ...
        let status_id = env
            .call_method(
                ...
                &[
                    JValue::Object(&status_name),
                    JValue::Object(&dimen_str),
                    JValue::Object(&android_str),
                ],
            )
            .map_err(|e| format!("getIdentifier(status): {}", e))?
            .i()
            .map_err(|e| format!("getIdentifier cast: {}", e))?;
        ...
        let status_px = if status_id > 0 {
            ...
            .i()
            .map_err(|e| format!("getDimensionPixelSize cast: {}", e))?
        } else {
            0
        };

        let nav_name: JObject = env
            .new_string("navigation_bar_height")
            .map_err(|e| format!("new_string: {}", e))?
            .into();
        let nav_id = env
            .call_method(
    ...
    pub(crate) fn move_task_to_back(&self) {
        ...
        unsafe {
            let Ok(vm) = jni::JavaVM::from_raw(vm_ptr as *mut jni::sys::JavaVM) else {
                return;
            };
    ...
    fn back_class_cache() -> &'static std::sync::atomic::AtomicPtr<()> {
        static CACHE: std::sync::atomic::AtomicPtr<()> =
    ...
    unsafe fn load_and_register_back(
        ...
        let buf = env
            .new_direct_byte_buffer(dex.as_ptr() as *mut u8, dex.len())
        ...
        let name: JObject = env
            .new_string("syngui.android.SynGuiBackHandler")
        ...
        let input_name: JObject = env
            .new_string("syngui.android.SynGuiInputHandler")
            .map_err(|e| format!("string: {}", e))?
            .into();
        let input_cls = env
            .call_method(
        ...
        let notif_name: JObject = env
            .new_string("syngui.android.SynGuiNotificationHandler")
            .map_err(|e| format!("string: {}", e))?
            .into();
        let notif_cls = env
            .call_method(
        ...
        let notif_global = env
            .new_global_ref(notif_cls_ref)
    ...
    unsafe fn poll_back_jni(
        ...
        let result = env
            .call_static_method(cls, "consumeBack", "()Z", &[])
            .ok()
            .and_then(|v| v.z().ok())
            .unwrap_or(false);
    ...
    pub(super) fn poll_input_handler(&mut self) {
        ...
        let vm_ptr = android_app.vm_as_ptr();
        let Ok(vm) = (unsafe { jni::JavaVM::from_raw(vm_ptr as *mut jni::sys::JavaVM) }) else {
            return;
        };
        let Ok(mut env) = vm.attach_current_thread_permanently() else {
            std::mem::forget(vm);
        ...
        let cls: jni::objects::JClass =
            unsafe { jni::objects::JObject::from_raw(ptr as jni::sys::jobject).into() };
        ...
        loop {
            ...
            if let Some(text) = event_str.strip_prefix("C:") {
                for _ in 0..self.composing_len {
                    self.tree
                        .handle_event(root_id, &Event::KeyDown(crate::input::Key::Backspace));
                }
                self.composing_len = 0;
                ...
                for _ in 0..after {
                    self.tree
                        .handle_event(root_id, &Event::KeyDown(crate::input::Key::Delete));
                }
            } else if event_str == "K:enter" {
                self.tree
                    .handle_event(root_id, &Event::KeyDown(crate::input::Key::Enter));
            }
        }
    ...
    pub(in crate::app) fn arm_keyboard_reveal(&mut self) {
        self.keyboard_reveal = self
            ...
            .current_focus()
            .or(self.tree.focused_element)
            .map(|element| super::KeyboardReveal {
                element,
                scrolled: false,
            });
    }
    ...
    unsafe fn toggle_soft_input_jni(
        ...
        if show {
            ...
            let token = env
                ...
                .l()
                .map_err(|e| format!("getWindowToken cast: {}", e))?;
            env.call_method(
                &imm,
    ...
    unsafe fn get_keyboard_height_jni(
        ...
        let visible_bottom = env
            ...
            .i()
            .map_err(|e| format!("rect.bottom cast: {}", e))?;
    ...
    pub(super) fn set_status_bar_light_icons(&self, light_icons: bool) {
        ...
        let activity_ptr = android_app.activity_as_ptr();
        if let Err(e) =
            unsafe { Self::set_status_bar_appearance_jni(vm_ptr, activity_ptr, light_icons) }
        {
    ...
    unsafe fn set_status_bar_appearance_jni(
        ...
        let controller = env
            ...
            .l()
            .map_err(|e| format!("getInsetsController cast: {}", e))?;
