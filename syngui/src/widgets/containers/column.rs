...
impl Element for ColumnElement {
    fn update(&mut self, widget: &dyn Widget, _ctx: &mut UpdateContext) {
        if let Some(col) = widget.as_any().downcast_ref::<Column>() {
            self.gap = col.gap;
            self.main_axis_alignment = col.main_axis_alignment;
            self.cross_axis_alignment = col.cross_axis_alignment;
            self.clip = col.clip;
            self.expand = col.expand;
            if col.width.is_some() {
                self.mss.width = col.width;
                self.builder_width = col.width;
            }
            if col.height.is_some() {
                self.mss.height = col.height;
                self.builder_height = col.height;
            }
