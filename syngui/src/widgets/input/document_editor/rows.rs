...
impl Element for TableBlockElement {
    ...
    fn layout(&mut self, constraints: Constraints) -> Size {
        ...
        // Столбцы делят ширину блока пропорционально содержимому — и когда
        // не влезают, и когда остаётся место: элемент и так занимает всю
        // ширину (рамка блока, ручка ⋮⋮), а сетка при естественной ширине
        // обрывалась на середине растянутого блока свободной раскладки.
