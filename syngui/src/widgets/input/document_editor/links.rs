...
/// Контекст построения врезки `![[…]]` — защита от рекурсии плюс
/// геометрия блока, которую хост не видит иначе.
...
pub struct EmbedCtx {
    ...
    /// Высота блока из свободной раскладки (`h`): врезка с собственной
    /// высотой (доска, диаграмма) занимает ровно её; `None` — хост берёт
    /// свой дефолт.
    pub height: Option<f32>,
}

/// Фабрика живых врезок: хост отдаёт виджет содержимого цели
/// (read-only страница, редактируемая доска, диаграмма). `None` —
...
pub trait EmbedFactory: Send + Sync {
    ...
    /// Врезка с собственной высотой: в свободной раскладке её можно
    /// тянуть за нижнюю кромку и задавать в свойствах (ключ `h`). У
    /// врезки страницы высота идёт от содержимого — по умолчанию `false`.
