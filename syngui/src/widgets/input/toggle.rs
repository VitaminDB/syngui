...
impl ToggleElement {
    fn target_props(&self) -> ResolvedProps {
        if self.is_on {
            if let Some(ref p) = self.style_checked {
                return p.clone();
            }
        }
        self.mss
            .style_normal
            .clone()
            .unwrap_or_else(|| ResolvedProps::new())
    }

    fn start_transition_to_current_state(&mut self) {
        if !self.mss.has_mss_styles || !self.mss.transition.has_specs() {
            return;
        }
        let Some(ref normal) = self.mss.style_normal else {
            return;
        };
