...
pub struct TabElement {
    ...
    icon: Option<String>,
    index: usize,
    selected_state: TabState,
    disabled: bool,
    closable: bool,
    on_close: Option<Arc<Mutex<dyn FnMut(usize) + Send>>>,
    is_selected: bool,
    bounds: Rect,
    close_button_bounds: Option<Rect>,
    hover: bool,
    hover_close: bool,
    focused: bool,
    classes: Vec<String>,
...
impl TabElement {
    fn start_transition_to_current_state(&mut self) {
        self.mss
            .start_transition_to(self.hover, false, false, self.is_selected);
    }
...
impl Element for TabElement {
    fn update(&mut self, widget: &dyn Widget, _ctx: &mut UpdateContext) {
        if let Some(tab) = widget.as_any().downcast_ref::<Tab>() {
            ...
            self.icon = tab.icon.clone();
            self.index = tab.index;
            self.selected_state = tab.selected_state;
            self.disabled = tab.disabled;
            self.closable = tab.closable;
    ...
    fn layout(&mut self, constraints: Constraints) -> Size {
        ...
        let badge_width = self
            ...
            .unwrap_or(0.0);
        let intrinsic =
            (pad_l + icon_width + text_width + badge_width + close_width + pad_r).max(80.0);
        let width = intrinsic
    ...
    fn build_display_list(&self, list: &mut DisplayList, _clip: Rect) {
        ...
        if !self.is_selected {
            let bottom_line = Rect::new(
                Point::new(
                    self.bounds.x(),
                    self.bounds.y() + self.bounds.size.height - border_width,
                ),
        ...
        if self.is_selected {
            ...
            let indicator = Rect::new(
                Point::new(
                    self.bounds.x() + inset,
                    self.bounds.y() + self.bounds.size.height - h,
                ),
        ...
        let badge_bg_width = self
            ...
            .unwrap_or(0.0);
        let badge_reserve = if badge_bg_width > 0.0 {
            badge_bg_width + 8.0
        } else {
            0.0
        };
        ...
        if self.closable {
            if let Some(close_rect) = self.close_button_bounds {
                let close_bg = if self.hover_close {
                    gray_200
                } else {
                    Color::TRANSPARENT
                };
    ...
    fn apply_transition_styles(
        ...
        self.mss.style_selected = selected
...
mod tests {
    ...
    fn height_is_clamped_to_parent_constraints() {
        ...
        assert_eq!(
            ...
            "без ограничения — высота по умолчанию"
        );
    }
    ...
    fn strip_padding_shrinks_bar_and_tabs() {
        ...
        assert_eq!(
            bar_b.origin.y, 8.0,
            "TabBar начинается под верхним отступом"
        );
        ...
        assert_eq!(
            bar_b.size.width,
            800.0 - 16.0,
            "width: 100% — на всю область содержимого"
        );
        ...
        for id in h.find_by_type_name("Tab") {
            ...
            assert_eq!(
                ...
                44.0,
                "низ вкладки — на нижней границе ленты"
            );
        }
    }
}
