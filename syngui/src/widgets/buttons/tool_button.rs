...
impl ToolButton {
    ...
    pub fn press_passthrough(mut self) -> Self {
        self.press_passthrough = true;
        self
    }
    ...
    pub fn on_click_with_bounds(
        mut self,
        callback: impl FnMut(Point, Rect) + Send + 'static,
    ) -> Self {
...
impl Element for ToolButtonElement {
    ...
    fn build_display_list(&self, list: &mut DisplayList, _clip: Rect) {
        ...
        let (bg, icon_color) = if self.mss.has_mss_styles {
            ...
            let bg = if self.disabled {
                ...
                default_bg.darken(0.05)
            } else if self.hover || self.active {
                default_bg
            } else {
                Color::TRANSPARENT
            };
            let ic = if self.disabled {
                fg.with_alpha(0.4)
            } else if self.active {
                accent
            } else {
    ...
    fn post_build_display_list(&self, list: &mut DisplayList, clip: Rect) {
        ...
        let Some(text) = self.tooltip.as_deref() else {
            return;
        };
        ...
        list.push_shadow(
            tip,
            Color::from_hex("#000000").with_alpha(0.35),
            12.0,
            (0.0, 4.0),
            [radius; 4],
        );
        ...
        let text_rect = Rect::new(
            ...
            Size::new(
                (tip.size.width - 16.0).max(0.0),
                (tip.size.height - 12.0).max(0.0),
            ),
        );
        list.push_text_styled(
            text,
