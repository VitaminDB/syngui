...
pub fn render_legend(
    ...
    let total_width: f32 = series
        ...
        .map(|(i, s)| {
            swatch_size
                + swatch_text_gap
                + estimate_text_width(&s.name, legend_font_size, text_measure)
    ...
    for (i, s) in series.iter().enumerate() {
        let opacity = if i < visibility.len() {
            visibility[i]
        } else {
            1.0
        };
        ...
        let swatch_rect = Rect::new(Point::new(x, center_y), Size::new(swatch_size, swatch_size));
...
pub fn render_legend_items(
    ...
    let total_width: f32 = names
        ...
        .map(|(i, name)| {
            swatch_size
                + swatch_text_gap
                + estimate_text_width(name, legend_font_size, text_measure)
