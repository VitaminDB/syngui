...
impl Element for CardElement {
    ...
    fn build_display_list(&self, list: &mut DisplayList, _clip: Rect) {
        ...
        let radii = self
            .mss
            .border_radius_resolved(ref_size, self.border_radius);
    ...
    fn apply_computed_style(&mut self, style: &ComputedStyle) {
        ...
        if let Some(p) = self.mss.padding_right {
            self.padding_right = p;
        }
        if let Some(p) = self.mss.padding_top {
            self.padding_top = p;
        }
        if let Some(p) = self.mss.padding_bottom {
