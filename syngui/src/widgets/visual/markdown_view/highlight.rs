...
const LANGUAGE_MARKERS: &[(&str, &[(Marker, u32)])] = &[
    ("rust", &[
        (Has("fn "), 3), (Has("let mut "), 3), (Has("println!"), 4), (Has("&str"), 3),
        (Has("impl "), 3), (Has("pub fn"), 3), (Has("match "), 2), (Has("Some("), 2),
        (Has("Option<"), 3), (Has("Vec<"), 3), (Has("#[derive"), 4), (Has("&self"), 3),
        (Has(".unwrap()"), 3), (Has("use std"), 4), (Has("&mut "), 3), (Has("-> "), 1),
        (Has("::"), 1), (Has("let "), 1), (Has("struct "), 1), (Has("enum "), 1),
        (Has("String"), 1), (Has("Result<"), 3), (Has("Ok("), 2), (Has("Err("), 2),
    ]),
    ("python", &[
        (Starts("def "), 3), (Starts("import "), 2), (Starts("from "), 1), (Has("print("), 3),
        (Has("self."), 2), (Starts("elif "), 3), (Has("__init__"), 4), (Starts("class "), 1),
        (Ends(":"), 1), (Has("lambda "), 2), (Has(" is not "), 2), (Has("range("), 2),
        (Has("len("), 1), (Starts("if __name__"), 5), (Has("None"), 1), (Has("True"), 1),
        (Has("False"), 1), (Starts("return "), 1), (Has(" in "), 1),
    ]),
    ("javascript", &[
        (Has("const "), 2), (Has("function "), 3), (Has("=>"), 2), (Has("console.log"), 4),
        (Has("var "), 2), (Has("==="), 3), (Has("!=="), 3), (Has("document."), 3),
        (Has("require("), 3), (Starts("export "), 2), (Has("async "), 1), (Has("await "), 1),
        (Has("let "), 1), (Has("null"), 1), (Has("undefined"), 3), (Has("this."), 1),
        (Has(".then("), 2), (Has("typeof "), 2), (Has("new "), 1), (Has("interface "), 2),
        (Has(": string"), 2), (Has(": number"), 2), (Has("import "), 1),
    ]),
    ("json", &[
        (Doc("{"), 3), (Doc("["), 2), (Has("\": "), 2), (Has("\":"), 1), (Has("null"), 1),
        (Has("true"), 1), (Has("false"), 1), (Ends(","), 1),
    ]),
    ("bash", &[
        (Doc("#!"), 5), (Has("#!/bin"), 5), (Starts("echo "), 2), (Starts("cd "), 2),
        (Starts("export "), 1), (Starts("sudo "), 3), (Starts("$ "), 3), (Has(" | "), 1),
        (Has("&&"), 1), (Starts("fi"), 2), (Starts("done"), 2), (Starts("esac"), 3),
        (Has("$("), 2), (Has("${"), 2), (Starts("if ["), 3), (Starts("for "), 1),
        (Starts("cargo "), 2), (Starts("git "), 2), (Starts("npm "), 2), (Starts("pip "), 2),
        (Starts("apt "), 2), (Starts("pacman "), 2), (Starts("ls "), 2), (Starts("mkdir "), 2),
        (Starts("rm "), 2), (Starts("cp "), 1), (Starts("mv "), 1), (Starts("curl "), 2),
        (Starts("chmod "), 2), (Has(" --"), 1), (Starts("then"), 2), (Starts("else"), 1),
    ]),
    ("html", &[
        (Doc("<!DOCTYPE"), 5), (Doc("<!doctype"), 5), (Doc("<html"), 5), (Doc("<?xml"), 5),
        (Doc("<"), 1), (Has("<div"), 3), (Has("</"), 2), (Has("<span"), 2), (Has("<p>"), 2),
        (Has("<a "), 2), (Has("<script"), 3), (Has("<body"), 3), (Has("<head"), 3),
        (Has("<ul"), 1), (Has("<li"), 1), (Has("<h1"), 2), (Has("/>"), 1), (Has("<svg"), 3),
        (Has("<button"), 3), (Has("<input"), 3), (Has("<table"), 2),
    ]),
    ("sql", &[
        (HasCi("SELECT "), 4), (HasCi(" FROM "), 3), (HasCi(" WHERE "), 3),
        (HasCi("INSERT INTO"), 4), (HasCi("CREATE TABLE"), 4), (HasCi(" JOIN "), 3),
        (HasCi("GROUP BY"), 3), (HasCi("ORDER BY"), 3), (HasCi("UPDATE "), 1),
        (HasCi(" SET "), 1), (HasCi("DELETE FROM"), 4), (HasCi("PRIMARY KEY"), 4),
        (HasCi("VARCHAR"), 3), (HasCi("ALTER TABLE"), 4), (HasCi(" VALUES "), 2),
    ]),
    ("cpp", &[
        (Starts("#include"), 5), (Has("int main("), 5), (Has("printf("), 3), (Has("std::"), 4),
        (Has("cout <<"), 4), (Has("cout<<"), 4), (Has("#define"), 3), (Has("nullptr"), 4),
        (Has("void "), 2), (Has("char *"), 2), (Has("char*"), 2), (Has("using namespace"), 5),
        (Has("template<"), 3), (Has("template <"), 3), (Has("int "), 1), (Has("return 0;"), 3),
        (Has("->"), 1), (Has("#pragma"), 3), (Has("size_t"), 2), (Has("malloc("), 3),
        (Has("scanf("), 3), (Has("std::vector"), 3), (Has("public:"), 3), (Has("private:"), 3),
    ]),
    ("go", &[
        (Starts("package "), 3), (Has("func "), 4), (Has("fmt."), 4), (Has(":="), 3),
        (Has("import ("), 3), (Has("func main()"), 5), (Has("chan "), 3), (Has("err != nil"), 5),
        (Has("defer "), 3), (Has("struct {"), 2), (Has("interface {"), 2), (Has("go "), 1),
    ]),
    ("css", &[
        (Has("px;"), 3), (Has("color:"), 3), (Has("margin"), 2), (Has("padding"), 2),
        (Has("display:"), 3), (Has("background"), 2), (Has("font-size"), 3), (Has("border"), 1),
        (Has("@media"), 4), (Has("!important"), 4), (Has("rem;"), 2), (Has("em;"), 1),
        (Has("%;"), 1), (Has("width:"), 2), (Has("height:"), 2), (Starts("."), 1),
        (Has(":hover"), 4), (Has("flex"), 1),
    ]),
];
...
mod guess_tests {
    ...
    fn plain_text_stays_plain() {
        ...
        assert_eq!(guess_language("Hello, world. This is a sentence about nothing."), None);
        // Вывод инструмента: список файлов, лог — не код.
        assert_eq!(guess_language("src/main.rs\nsrc/lib.rs\nCargo.toml"), None);
        assert_eq!(guess_language("[INFO] started\n[WARN] slow query 120 ms\n[INFO] done"), None);
    }
}
