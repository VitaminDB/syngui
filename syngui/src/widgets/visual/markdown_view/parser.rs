...
impl ParseContext {
    ...
    fn process(&mut self, event: Event) {
        match event {
            ...
            Event::Start(Tag::CodeBlock(kind)) => {
                ...
                self.stack.push(Frame::Paragraph {
                    inlines: vec![MdInline::Text(lang.unwrap_or_default())],
                });
            }
            ...
            Event::Start(Tag::Image {
                ...
                self.stack.push(Frame::Link {
                    url: dest_url.to_string(),
                    inlines: vec![MdInline::Text(title.to_string())],
                });
            }
        ...
        match tag {
            ...
            TagEnd::Heading(_) => {
                if let Some(Frame::Heading { level, inlines }) = self.stack.pop() {
                    self.emit_block(MdBlock::Heading {
                        level,
                        inlines,
                        id: None,
                    });
            ...
            TagEnd::CodeBlock => {
                if let Some(Frame::Paragraph { inlines }) = self.stack.pop() {
                    ...
                    let code: String = inlines
                        .iter()
                        .skip(1)
                        .filter_map(|i| {
                            if let MdInline::Text(t) = i {
                                Some(t.as_str())
                            } else {
                                None
                            }
                        ...
                        .collect();
                    self.emit_block(MdBlock::CodeBlock {
                        language: lang,
                        code,
                    });
                }
            ...
            TagEnd::Link => {
                if let Some(Frame::Link { url, inlines }) = self.stack.pop() {
                    self.push_inline(MdInline::Link {
                        children: inlines,
                        url,
                    });
                }
    ...
    fn push_inline(&mut self, inline: MdInline) {
        ...
        if let Some(MdBlock::Paragraph { inlines }) = self.root_blocks.last_mut() {
            ...
            self.root_blocks.push(MdBlock::Paragraph {
                inlines: vec![inline],
            });
        }
    }
