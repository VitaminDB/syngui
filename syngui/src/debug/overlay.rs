...
impl DebugOverlay {
    ...
    pub fn build_display_list(&self, display_list: &mut DisplayList) {
        ...
        let fps_color = if current_fps >= 50.0 {
            green
        ...
        let lines: [(String, Color); 9] = [
            ...
            (
                format!(
                    "Verts: {}  Elems: {}",
                    self.last_stats.vertex_count, self.last_stats.element_count
                ),
                white,
            ),
            (
                format!(
                    "Glyphs: {}  Atlas: {} KB",
                    self.last_stats.font_atlas_glyphs, self.last_stats.font_atlas_mem_kb
                ),
                white,
            ),
            (
                format!("DL Commands: {}", self.last_stats.display_list_commands),
                white,
            ),
        ];
