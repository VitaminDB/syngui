...
fn nested_center_alignment() {
    let widget = Stack::new().fit(StackFit::Expand).child(
        DecoratedBox::new()
            .class("layer")
            .child(DecoratedBox::new().class("badge").child(Icon::new("x").class("glyph"))),
    );
    ...
    assert_eq!(
        (layer.origin.x, layer.origin.y, layer.size.width, layer.size.height),
        (0.0, 0.0, 1920.0, 1080.0),
        "layer {layer:?}"
    );
    let glyph = h.element_bounds(h.find_by_class("glyph")[0]);
    assert_eq!(
        (badge.origin.x, badge.origin.y, badge.size.width, badge.size.height),
        (894.0, 474.0, 132.0, 132.0),
        "badge {badge:?}"
    );
    assert_eq!(
        (glyph.origin.x, glyph.origin.y, glyph.size.width, glyph.size.height),
...
fn end_alignment_respects_padding() {
    ...
    assert_eq!(
        (inner.origin.x, inner.origin.y, inner.size.width, inner.size.height),
