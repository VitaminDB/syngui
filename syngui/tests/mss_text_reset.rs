...
fn color_does_not_stick_after_class_change() {
    ...
    assert!(
        ...
        "предусловие: .err красит текст, получили {color:?}"
    );
    ...
    assert_eq!(
        weight, 700,
        "стиль нового класса при этом должен примениться"
    );
}
...
fn builder_values_survive_the_reset() {
    ...
    let widget: Box<dyn Widget> = Box::new(Text::new("hello").color(blue).bold().class("err"));
    let mut h = TestHarness::new(widget);
    ...
    assert!(
        color.r > 0.9,
        "предусловие: MSS перебивает цвет билдера, получили {color:?}"
    );
    assert_eq!(
        weight, 400,
        "предусловие: MSS перебивает font-weight билдера"
    );
...
fn inherited_color_survives_the_reset() {
    ...
    let widget: Box<dyn Widget> = Box::new(Column::new().class("panel").child(Text::new("hello")));
    let mut h = TestHarness::new(widget);
