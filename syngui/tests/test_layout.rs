...
fn column_with_gap_sizes_correctly() {
    ...
    assert!(
        (size.height - 110.0).abs() < 1.0,
        "Expected height ~110, got {}",
        size.height
    );
}
...
fn row_with_gap_sizes_correctly() {
    ...
    assert!(
        (size.width - 210.0).abs() < 1.0,
        "Expected width ~210, got {}",
        size.width
    );
}
...
fn padding_adds_to_child_size() {
    ...
    assert!(
        (size.width - 140.0).abs() < 1.0,
        "Expected width ~140, got {}",
        size.width
    );
    assert!(
        (size.height - 90.0).abs() < 1.0,
        "Expected height ~90, got {}",
        size.height
    );
}
...
fn find_elements_by_type() {
    ...
    assert_eq!(
        texts.len(),
        2,
        "Expected 2 Text elements, found {}",
        texts.len()
    );
